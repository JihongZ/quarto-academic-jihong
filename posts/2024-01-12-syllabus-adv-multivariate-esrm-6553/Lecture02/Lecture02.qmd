...
-   Question here: WHY USING BERNOULLI DISTRIBUTION (*Jacob Bernoulli, 1654-1705*)?
...
p1[which.max(Likelihood)] # p1 = 0.6 = 2 / 5
...
-   The Beta distribution has a variance of $\frac{\alpha\beta}{(\alpha+\beta)^2 (\alpha + \beta + 1)}$
    ...
    alpha_beta_variance_plot %>% 
      ...
      geom_text(aes(label = Alpha_Beta)) +
...
-   The smaller prior variance means the prior is more [informative]{.underline}
    ...
    -   [UniInformative]{.underline} priors are those that have relatively large variances
