...
SessionInfo <- SessionTBL |> 
  mutate(
    Session = ifelse(str_detect(Session_Content, "Session"), Session_Content, ""),
    Presentation = ifelse(!str_detect(Session_Content, "Session"), Session_Content, ""),
  ) |> 
...
Topics |> 
  ...
  geom_text(aes(x = fct_reorder(Topic, Frequency), y = Frequency, label = paste0(Frequency, " / ", sum(!is.na(SessionInfo$Presentation))) ), nudge_y = 0.3) +
  labs(x = "Topics") +
